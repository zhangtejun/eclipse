        ...
        $(function () {
            ...
            btn.cropperUpload({
                ...
                cropperParam: {//Jcrop参数设置，除onChange和onSelect不要使用，其他属性都可用
                    maxSize: [800, 800],//不要小于50，如maxSize:[40,24]
                ...
                perviewImgStyle: { width: '200px', height: '200px', border: '1px solid #ebebeb' }//设置预览图片的样式  
            });
...
<form action="cutPic.action" method="post">  
    点击  
    <input type="" name="image.x" id="x"/>  
    <input type="" name="image.y" id="y"/>  
    <input type="" name="image.width" id="width"/>  
    <input type="" name="image.height" id="height"/>  
	            ...
	            window.setTimeout(function () {
	                $(upload.getIframeContentDocument().body).find("input[type='file']").change(function () {
	                    ...
	                    if (upload.settings.perviewImageElementId) {
	                        ...
	                        var main1=main;
// 	                        $("#preview").append(main1);
	                        var div = $(main).children("div").get(0);

	                        perviewImage.beginPerview(this, div, upload.getIframeContentDocument(), upload);
	                        perviewImage.beginPerview(this, $(main1).children("div").get(0), upload.getIframeContentDocument(), upload);
	                    }
	                });
	...
	UploadAssist.prototype = {
	    ...
	    chooseFile: function () {
	        if (this.settings.perviewImageElementId) {
	            $("#" + this.settings.perviewImageElementId).empty();//empty() 方法从被选元素移除所有内容，包括所有文本和子节点。
	        }
...
// 	                });
	            ...
	            function setImageDisplay() {
	                ...
	                $(div).Jcrop({
	                	  ...
	                	  onChange: upload.setImageCropperObj(obj),  
	                      onSelect: upload.setImageCropperObj(obj),  
	                      aspectRatio: 1  
	                },function(){  debugger
	                    // Use the API to get the real image size  
	                    var bounds = this.getBounds();  
	    ...
	    //按比例缩放图片
	    clacImgZoomParam: function (maxWidth, maxHeight, width, height) {debugger
	    ...
	    createPreviewElement: function (/*关闭图片名称*/name, /*上传时的文件名*/file, /*预览时的样式*/style) {
	        var img = document.createElement("div");debugger
	        img.title = file;
	...
	function updatePreview(c){
		...
		if (parseInt(c.w) > 0){	
			$('#preview1').css({
				...
				marginLeft: '-' + Math.round(800 / c.w * c.x) + 'px',
				marginTop: '-' + Math.round(800 / c.h * c.y) + 'px'
			});
